...
int calculInaltime(Nod* rad) {
    if (rad) {
        return maxim(calculInaltime(rad->st), calculInaltime(rad->dr)) + 1;
    }
    return 0;
}

int calculDiferentaInaltime(Nod* rad) {
    if (rad) {
...
void adaugaMasinaInArbore(Nod** rad, Masina m1) {
    ...
    int diferentaInaltimi = calculDiferentaInaltime(*rad);
    if (diferentaInaltimi == 2) {
        if (calculDiferentaInaltime((*rad)->st) == -1) {
    ...
    if (diferentaInaltimi == -2) {
        if (calculDiferentaInaltime((*rad)->dr) == 1) {
